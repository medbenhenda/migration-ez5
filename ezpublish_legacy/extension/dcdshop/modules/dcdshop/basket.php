...
* Entity Selection
*
* Training entity is based on country of session and countries selected in the company
*
* Currency is based on training entity
* 
*/
$basket_country = dcdShop::setBasketEntity($basket);

/* 
 * End Of Currency Check 
 */

$basket->updatePrices(); // Update the prices. Transaction not necessary.

$addtobasketafterclear = false;
if ( $http->hasPostVariable( "ActionClearAndAddToBasket" ) )
{
	$basket->remove();
}


if ( $http->hasPostVariable( "ActionAddToBasket" ) || $http->hasPostVariable( "ActionClearAndAddToBasket" ) )
{
...
// Apply discounts
$discount_ids = explode("|", eZPreferences::value( 'user_discount_code' ));

if(isset( $discount_ids[0] ) &&  $discount_ids[0] != null &&  $discount_ids[0] != '')
{
	$discount_info = dcdShop::getDiscountsInfo( $discount_ids );
	$discount_applied = dcdShop::applyDiscounts( $discount_ids  );
	
	$tpl->setVariable( 'discount_info', $discount_info );
}
