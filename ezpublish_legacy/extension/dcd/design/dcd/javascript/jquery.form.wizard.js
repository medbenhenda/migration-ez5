...
(function($){
	$.widget("ui.formwizard", {
		...
		_init: function() {
			...
			//Jinit Patel - Start comment
			// Fetch dynamic id of register, login, forgot based on url enter
			var hashstring = window.location.hash;
			var hasharray = hashstring.split("=");
			this.firstStep = hasharray[1];//this.steps.eq(0).attr("id");
			if(this.firstStep	== undefined){
				this.firstStep = this.steps.eq(0).attr("id");	
			}
			//End comment - Jinit Patel
			...
			if(this.options.historyEnabled){
				$(window).bind('hashchange', undefined, function(event){
					...
					//added by jinit patel
					//Form switcher
					if(loggedin == true && hashStep != 'loggedin'){
						hashStep = "loggedin";
					}
					if(loggedin == false){
						if(hashStep == "loggedin")
							{
								hashStep = "first";
							}
						else{
					...
					//Comment end - added by jinit patel
					//Form switcher
