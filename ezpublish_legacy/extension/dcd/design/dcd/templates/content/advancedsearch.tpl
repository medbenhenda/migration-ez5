...
{/if}
	...
	<form action={"dcpro/advancedsearch/"|ezurl} method="get">
		<h2>{'Advanced search'|i18n( 'design/ezwebin/content/advancedsearch' )}</h2>
	
		<article class="course_list">
			<div class="block search_block">
				<label class="search_label">{'Search all the words'|i18n( 'design/ezwebin/content/advancedsearch' )}</label><div class="labelbreak"></div>
				<input id="query" class="box" type="text" size="40" name="SearchText" value="{$full_search_text|wash}" />
			</div>
			<div class="block search_block">
				<label>{'Search the exact phrase'|i18n( 'design/ezwebin/content/advancedsearch' )}</label><div class="labelbreak"></div>
				<input id="query" class="box" type="text" size="40" name="PhraseSearchText" value="{$phrase_search_text|wash}" />
			</div>
	...
	{include name=navigator
			...
			{if $search_result|count()}
				{foreach $search_result as $element}
				   {node_view_gui view=preview content_node=$element}
				{/foreach}
			{/if}
...
{set scope=global persistent_variable=hash( 'nav_parent_id', 64,
