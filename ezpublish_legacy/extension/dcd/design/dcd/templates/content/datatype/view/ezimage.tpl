{* 
Override of standard ezimage file
...
{if $image_content.is_valid}
    ...
    {if and( is_set( $image ), $image )}
        ...
        <img {if and($image_css|ne(''), is_set($image_css))} class={$image_css}{/if} src={$image.url|ezroot} width="{$image.width}" height="{$image.height}" {if $hspace}hspace="{$hspace}"{/if} style="{$inline_style}" alt="{$alt_text|wash(xhtml)}" title="{$title|wash(xhtml)}" />
        {if $href}</a>{/if}
    {/if}
