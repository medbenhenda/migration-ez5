...
// $searchContentClassAttributeID = array();

$search_delimiter = '';

if ( $http->hasVariable( 'disciplines' ) )
{
	$disciplines = $http->variable( 'disciplines' );
	if ( $disciplines != "" )
	{
		$searchContentClassAttributeID = array( 379 );
		
		$searchText = $disciplines ;
		$phraseSearchText = $disciplines;
		$search_delimiter = ' ';
...
{
	$course_type = $http->variable( 'course_type' );
	if ( $course_type != "" )
	{
		if(count($searchContentClassAttributeID) >= 1){
			 array_push($searchContentClassAttributeID, 378 );
		}
		else{
			$searchContentClassAttributeID = array( 378 );
		}
		
		$searchText .=  $search_delimiter . $course_type ;
		$phraseSearchText = $course_type;
